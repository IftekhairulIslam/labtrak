...
export function PreEntryPractice() {
  ...
  const currentCompleted = currentTests.filter((item) =>
    completedIds.has(`${item.code}-${item.synonym}`),
  );
  const allCurrentDone =
    currentTests.length > 0 && currentCompleted.length === currentTests.length;
  const allSetsDone =
    sets.length > 0 && setIndex >= sets.length - 1 && allCurrentDone;
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (matched) {
      ...
      setCompletedIds((prev) =>
        new Set(prev).add(`${matched.code}-${matched.synonym}`),
      );
      setInput("");
    }
  };
